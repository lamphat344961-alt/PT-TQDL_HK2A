...
bp <- barplot(scores, 
              ...
              col = rainbow(5),  # dùng rainbow(5) để mỗi môn một màu tự động
              ylim = c(0, 10),
              ylab = "Điểm số")
...
hist(exam_scores, 
     breaks = 10, 
     main = "Phân phối điểm thi sinh viên", 
     xlab = "Điểm số", 
...
hist(exam_scores, breaks = 20, main = "Histogram với 20 bins", col = "lightgreen")
# Nhận xét: 20 bins chi tiết hơn 
...
boxplot(Petal.Length ~ Species, 
        data = iris, 
        main = "Chiều dài cánh hoa theo loài", 
        col = c("pink", "lightblue", "lightgreen"),
        ylab = "Chiều dài (cm)")

# Trả lời:
# - Loài có petal dài nhất: Virginica (hộp nằm cao nhất).
# - Loài có độ biến thiên lớn nhất: Virginica (chiều cao hộp và râu dài nhất).
# - Có outliers xuất hiện ở loài Setosa (các dấu chấm nằm ngoài râu).
...
plot(mtcars$hp, mtcars$mpg, 
     col = mtcars$cyl, 
     pch = 19, 
     main = "Mối quan hệ mã lực và tiêu thụ xăng",
     xlab = "Mã lực (hp)", 
     ylab = "Dặm/gallon (mpg)")
...
# Nhận xét: Mã lực càng cao (hp tăng) thì tiêu thụ xăng càng nhiều (mpg giảm).
...
par(mfrow = c(2, 2)) # Chia màn hình 2x2

# 1. Histogram
hist(mtcars$hp, main = "Phân phối HP", col = "gold")

# 2. Boxplot
boxplot(mtcars$hp, main = "Boxplot HP", col = "orange")

# 3. Boxplot theo nhóm
boxplot(hp ~ cyl, data = mtcars, main = "HP theo Cylinders", col = rainbow(3))

# 4. Scatter plot
plot(mtcars$hp, mtcars$mpg, main = "HP vs MPG", pch = 19, col = "red")

par(mfrow = c(1, 1)) # Reset về mặc định
...
colnames(data_matrix) <- products
rownames(data_matrix) <- c("Quý 1", "Quý 2", "Quý 3", "Quý 4")
...
barplot(data_matrix, 
        ...
        main = "Doanh thu các sản phẩm theo Quý",
...
plot(1:4, data_matrix[, 1], type = "b", col = "blue", ylim = c(100, 250),
     xlab = "Quý", ylab = "Doanh thu", main = "Xu hướng doanh thu từng SP", pch = 19)
# Vẽ thêm các sản phẩm khác
lines(1:4, data_matrix[, 2], type = "b", col = "red", pch = 19)
lines(1:4, data_matrix[, 3], type = "b", col = "darkgreen", pch = 19)
lines(1:4, data_matrix[, 4], type = "b", col = "purple", pch = 19)

# 3. Tính tổng doanh thu mỗi quý và vẽ bar chart
tong_quy <- rowSums(data_matrix)
barplot(tong_quy, col = "skyblue", main = "Tổng doanh thu theo Quý", ylab = "Triệu đồng")

# 4.
# Thiết lập khung hình 2 hàng, 2 cột
par(mfrow = c(2, 2))

# --- Ô 1: Grouped Bar Chart ---
barplot(data_matrix, beside = TRUE, col = terrain.colors(4),
        main = "1. Doanh thu theo Quý", font.main = 2)

# --- Ô 2: Line Plot tất cả sản phẩm ---
plot(1:4, data_matrix[, 1], type = "b", col = "blue", ylim = c(100, 230),
     xlab = "Quý", ylab = "Doanh thu", main = "2. Xu hướng sản phẩm", pch = 19)
lines(1:4, data_matrix[, 2], type = "b", col = "red", pch = 19)
lines(1:4, data_matrix[, 3], type = "b", col = "darkgreen", pch = 19)
lines(1:4, data_matrix[, 4], type = "b", col = "purple", pch = 19)
legend("topleft", legend = products, col = c("blue", "red", "darkgreen", "purple"), 
       lty = 1, cex = 0.6)

# --- Ô 3: Pie Chart tổng doanh thu mỗi quý ---
tong_quy <- rowSums(data_matrix)
labels_quy <- paste(names(tong_quy), "\n", round(tong_quy/sum(tong_quy)*100, 1), "%")
pie(tong_quy, labels = labels_quy, col = rainbow(4), main = "3. Tỷ trọng theo Quý")

# --- Ô 4: Bar Chart tổng doanh thu mỗi sản phẩm ---
tong_sp <- colSums(data_matrix)
barplot(tong_sp, names.arg = products, col = "lightcoral", 
        main = "4. Tổng doanh thu mỗi SP", las = 2, cex.names = 0.8)

# Trả màn hình về mặc định
par(mfrow = c(1, 1))
